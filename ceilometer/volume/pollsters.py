...
class SnapshotSizePollster(plugin.CentralPollster):

    LOG = log.getLogger(__name__ + '.snapshotsize')

    def _get_snapshots(self, manager):
        ksclient = manager.keystone
        endpoint = ksclient.service_catalog.url_for(
            service_type='volume',
            endpoint_type=cfg.CONF.service_credentials.os_endpoint_type)
        ...
        return cinder.volume_snapshots.list(search_opts={'all_tenants': True})

    def _iter_snapshots(self, manager, cache):
        if 'snapshots' not in cache:
            cache['snapshots'] = list(self._get_snapshots(manager))
        return iter(cache['snapshots'])

    def get_samples(self, manager, cache):
        for snapshot in self._iter_snapshots(manager, cache):
            self.LOG.info("SNAPSHOT USAGE %s" % snapshot)
            yield sample.Sample(
                name='snapshot.size',
                type=sample.TYPE_GAUGE,
                unit='GB',
                ...
                user_id=None,
                project_id=getattr(snapshot, 'os-vol-tenant-attr:tenant_id'),
